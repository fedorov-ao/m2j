...
class Matrix:
    ...
    def __pow__(self, other):
        ...
        if not isinstance(other, int) or other < 1.0:
    ...
    def cofactor(self, row, col):
        ...
        return pow(-1.0, row + col) * self.minor(row, col)
    ...
    def len(self):
        ...
        return math.sqrt(sum(e ** 2.0 for _, _, e in self))

    def dir(self):
        """ Vectors only. Returns a unit vector in the same direction. """
        return (1.0 / self.len()) * self
...
def get_row_echelon_form(matrix, mirror=None):
    ...
    for top_row in range(matrix.numrows):
        ...
        # Make this entry '1'.
        if matrix[top_row][col] != 1.0:
            multiplier = 1.0 / matrix[top_row][col]
            matrix.rowop_multiply(top_row, multiplier)
            matrix[top_row][col] = 1.0 # assign directly in case of rounding errors
