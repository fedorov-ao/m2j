...
class ParserState:
  ...
  def get_obj(self, name, **kwargs):
    ...
    objectName0 = objectName[0]
    obj = None
    i = { "self" : 0, "parent" : 1, "root" : -1 }.get(objectName0, None)
    if i is not None:
      obj = self.at("eps", i)
    else:
      i = 0
...
class SensSetEP:
  ...
  def print_set_(self):
    items = list(self.currentSet_.items())
    items.reverse()
    s = ""
    for i in items:
...
class SegmentFunc:
  ...
  def get_points(self):
    self.update_points_()
    return list(zip(self.x_, self.y_))
...
class FuncEditorWidget(tk.Frame):
  def __init__(self, **kwargs):
    ...
    self.devBox_ = tkinter.ttk.Combobox(master=self, width=self.widths_["devbox"])
    self.devBox_.pack(side="left")
    self.axisBox_ = tkinter.ttk.Combobox(master=self, width=self.widths_["axisbox"])
    self.axisBox_.pack(side="left")
    self.funcBox_ = tkinter.ttk.Combobox(master=self, width=self.widths_["funcbox"])
  ...
  def select_dev_(self, event=None):
    v = self.devBox_.get()
    self.currentDev_ = v
    values = list(self.funcs_[v].keys())
  ...
  def select_axis_(self, event=None):
    v = self.axisBox_.get()
    self.currentAxis_ = v
    values=list(self.funcs_[self.currentDev_][self.currentAxis_].keys())
...
def make_parser():
  ...
  def parseComboboxWidget(cfg, state):
    ...
    import tkinter.ttk
    widget = tkinter.ttk.Combobox(**kwargs)
    manager = bind_box_to_var(widget, cfg, state)
    def command(*args, **kwargs):
      manager.update_var()
  ...
  def parsePresetWidget(cfg, state):
    ...
    presetCfg = get_nested_d(presets, presetName, None)
    if presetCfg is None:
      raise ParseError(cfg, state.get_path(cfg), "preset '{}' does not exist; available presets are: '{}'".format(presetName, list(presets.keys())))
    state.push_args(cfg)
